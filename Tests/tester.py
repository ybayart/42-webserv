...
url2 = "http://localhost:8080/f";
list_url = [
	url,
	url2
];
body = "num1=12&num2=145";
headers = {
...
def get_url(url, headers):
	...
	r1 = s.get(url2);
	r2 = s.post(url);
	if r1.text == "404 Not Found":
		print("ok get");
	else:
		print("error get");
	if r1.text == "403 Not Allowed":
		print("ok post");
	else:
		print("error post");
	# print(r.text);
...
with ThreadPoolExecutor(max_workers=max) as pool:
	future = pool.map(get_url, list_url, list_headers);
	future2 = pool.map(post_url, list_url, list_headers);
	# print(future.result());
